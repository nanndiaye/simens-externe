...
function ajouterPatient(){
	vart=tabUrl[0]+'public/chururgie/ajout-patient';
...
function visualiser(id) { 
	...
	var cle = id;
	var chemin = tabUrl[0] + 'public/chururgie/get-infos-vue-patient';
	$.ajax({
		type : 'POST',
		url : chemin,
		data : $(this).serialize(),
...
function getListeLits(id_salle){ 
	...
	$('#lit').html("");
	var chemin = tabUrl[0] + 'public/chururgie/liste-lits';
	$.ajax({
		type : 'POST',
		url : chemin,
...
function initialisationListeInfTri() {
	...
	oTableListeInfTri = $('#patientAdmisParInfirmierTri') .dataTable( {
						...
						"sAjaxSource" : "" + tabUrl[0] + "public/chururgie/liste-admission-infirmier-tri-ajax",
...
function raffarichirLaListePatientAdmisParInfirmierTri(){
	setTimeout(function(){
		...
		var chemin = tabUrl[0] + 'public/chururgie/get-nb-patient-admis-non-vu';
		$.ajax({
			type : 'POST',
			url : chemin,
			success : function(data) {
...
function admettreVersMedecin(id_patient, id_admission) { 
	...
	// R�cup�ration des donn�es du patient
	var chemin = tabUrl[0] + 'public/chururgie/get-infos-admission-par-infirmier-tri';
	$.ajax({
		type : 'POST',
		url : chemin,
