    ...
    function affichervue(idPatient, idAdmission){
        ...
        var chemin = tabUrl[0]+'public/urgence/vue-patient-admis';
        $.ajax({
            type: 'POST',
            url: chemin ,
            data: $(this).serialize(),  
	...
	/*setTimeout(function() {
		vart='<?php echo $this->basePath().'/urgence/rpu-patients';?>';
	    $(location).attr("href",vart);
	}, 60000);*/ //Une minute

	function consultation(idPatient, id_cons){
		vart=tabUrl[0]+'public/urgence/complement-consultation?id_patient='+idPatient+'&id_cons='+id_cons;
	    $(location).attr("href",vart);
	}

	function modifierConsultation(idPatient, id_cons){
		vart=tabUrl[0]+'public/urgence/maj-complement-consultation?id_patient='+idPatient+'&id_cons='+id_cons;
	    $(location).attr("href",vart);
	}
