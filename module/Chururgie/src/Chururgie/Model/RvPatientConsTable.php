...
class RvPatientConsTable
{
    ...
    public function updateRendezVous($infos_rv)
    {
       ...
       if ($infos_rv->date_rv && $infos_rv->heure_rv) {
        	$donnees = array(
        			'ID_CONS'=> $infos_rv->id_cons,
        			'NOTE'=>$infos_rv->motif_rv,
        			'DATE'=>$infos_rv->date_rv,
        			'DELAI'=>$infos_rv->delai_rv,
        			'HEURE'=>$infos_rv->heure_rv,
        	);
    ...
    // Liste des patients ayant Rendez-vous aujoud'hui
    
    public function getRVAujourdhui()
    {
    	...
    	join(array(
    			'p' => 'personne'
    	), 'pat.id_personne = p.id_personne', array(
    			'Nom' => 'NOM',
    			'Prenom' => 'PRENOM',
   ...
   // ->join(array('a' => 'admission'), 'p.ID_PERSONNE = a.id_patient', array('*'))
    ->	join(array(
    	...
    	->join(array(
    			'rv' => 'rendezvous_consultation'
    	), 'rv.ID_CONS = cons.ID_CONS', array(
    			'delai' => 'DELAI'
    	))
    		...
    		for ($i = 0; $i < count($aColumns); $i ++) {
    			if ($aColumns[$i] != ' ') {
    				...
    				if ($aColumns[$i] == 'Nom') {
    					...
    					$html = "<infoBulleVue> <a href='" . $tabURI[0] . "public/chururgie/admission-infos-patient-rv?id_cons=" . $aRow['Id_cons'] . "&id_patient=" . $aRow[$aColumns[$i]] . "'>";
    					$html .= "<img style='display: inline; margin-right: 20%;' src='" . $tabURI[0] . "public/images_icons/voir	.png' title='Info RV'></a></infoBulleVue>";
            ...
            for ($i = 0; $i < count($aColumns); $i ++) {
                if ($aColumns[$i] != ' ') {
                    ...
                    if ($aColumns[$i] == 'Nom') {
                        ...
                        $html = "<infoBulleVue> <a href='" . $tabURI[0] . "public/chururgie/modifier-infos-patient-rv?id_cons=" . $aRow['Id_cons'] . "&id_patient=" . $aRow[$aColumns[$i]] . "'>";
    ...
    public function getTousRVAConfirmer()
    {
            ...
            for ($i = 0; $i < count($aColumns); $i ++) {
                if ($aColumns[$i] != ' ') {
                    ...
                    if ($aColumns[$i] == 'Nom') {
                        ...
                        $html = "<infoBulleVue> <a href='" . $tabURI[0] . "public/chururgie/info-patient-rv?id_cons=" . $aRow['Id_cons'] . "&id_patient=" . $aRow[$aColumns[$i]] . "'>";
    ...
    public function AddRendezVousPatient($donneesRv)
    {
        ...
        //var_dump($donneesRv->id_cons);exit();
        $sql = new Sql($db);
       // $requete = $sql->delete('*')->from('rendezvous_consultation')->where('ID_CONS =>', $donneesRv->id_cons);
       // $result = $sql->prepareStatementForSqlObject($requete);
        
        $this->tableGateway->delete(array(
        		'ID_CONS' => $donneesRv->id_cons
        ));
     
       // var_dump($r);exit();
     
      //	var_dump($donneesRv->id_cons);exit();
        	$sQuery = $sql->insert()->into('rendezvous_consultation')
        	->values(array(
        			'ID_CONS'=> $donneesRv->id_cons,
        			'NOTE'=>$donneesRv->motif_rv,
        			'DATE'=>$donneesRv->date_rv,
        			'DELAI'=>$donneesRv->delai_rv,
        			'HEURE'=>$donneesRv->heure_rv,
        	));
